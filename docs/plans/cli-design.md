...
A checkup is a Python module containing a `hub` variable (a `CheckUp` instance) that defines the measurement configuration.
...
Metrics are configured via a YAML file passed to CheckUp:
...
hub = (
    CheckUp(config_path="checkup.yaml")
